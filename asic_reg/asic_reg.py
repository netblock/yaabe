...
#mode = 1
# Print out register names by the order of their address. The primary purpose is
# to autogenerate a C header that contains a list that's meant to be search upon

#mode = 2
# Print out bitfields in the style of PPATUI.

#mode = 3
# Print out the names of the bitfields for a PPATUI header.
...
def get_newest_index_name(addr:int, bitfield:bitfield):
	# typically, mm -> reg
...
# mode == 0
def bitfield_to_header(ip_name:str, versions:list, composite_bitfields:dict):
	header_header = """\
/*
...
# mode == 1
def bitfield_to_reglist(ip_name:str, versions:list, composite_bitfields:dict):
	# "name": [bf,bf,bf]
	header_header = """\
/*
autogenerated with asic_reg.py
...
# mode == 2
def bitfield_to_ppatui(ip_name:str, versions:list, composite_bitfields:dict):
	header_header = """\
/*
autogenerated with asic_reg.py
Considers %s versions: %s
...
"""
	...
	bf_string_start = """\
PPATUI_FUNCIFY(union, %s_%s, atui_nullstruct,
	(bios->raw_data, u8\"%s\",
		(ATUI_BIN, ATUI_BITFIELD, (
"""
	bf_string_mid_1 = "\t\t\t(u8\"%s\",%s%u,%u,"
	bf_string_mid_2 = "ATUI_DEC, (ATUI_NODESCR)"
	bf_string_mid_3 = "),\n"
	bf_string_end = """\
		)), (ATUI_NODESCR)
	)
...
"""
	out_text = "" # the final buffer
	mid_part1 = ""; mid_text = "" # text wrapping the bitfield lines
	bf_startvals = (); bf_midvals = () # for the %s
	ver_addr = {}

	versions.sort(key=lambda v: [int(n) for n in v.split("_")])
	ver_str = ", ".join(versions)
	out_text = header_header % (ip_name, ver_str)

	for name in composite_bitfields:
		for bf in composite_bitfields[name]:
			ver_addr = bf.ver_addr

			bf_startvals = (name, tuple(ver_addr)[0], name)
			out_text += bf_string_start % bf_startvals

			longest_named = bf.find_longest_named()
			len_longest = len(longest_named["name"])
			long_hi_double = (10 <= longest_named["hi"])
			for entry in bf.entries: #  name, hi,lo, ATUI_DEC...
				len_entry = len(entry["name"])
				num_spaces = (
					len_longest - len_entry + 1 # push out, replace, extra
					+      (long_hi_double and (10 > entry["hi"]))
				...
				# wrap text
				mid_part1 = (bf_string_mid_1 % bf_midvals)
				mid_text = mid_part1 + " " + bf_string_mid_2 + bf_string_mid_3
				if 71 >= len(mid_text):
					out_text += mid_text
				else:
					out_text += (
			...
			out_text += bf_string_end # )), (ATUI_NODESCR) ) )
...
# mode == 3
def bitfield_to_include(ip_name:str, versions:list, composite_bitfields:dict):
	# "name": [bf,bf,bf]
	header_header = """\
/*
autogenerated with asic_reg.py
...
#ifndef %s_ATUI_INCLUDE_H
#define %s_ATUI_INCLUDE_H

"""
	preprossor_str = "PPATUI_HEADERIFY(%s_%s);\n"
	header_ender = """\
#endif
"""
	out_text = "" # the final buffer
	...
	for name in composite_bitfields:
		for bf in composite_bitfields[name]:
			out_text += preprossor_str % (name, tuple(bf.ver_addr)[0])
		out_text += "\n"
...
def main(argc:int, argv:list):
	...
	if mode in {0,1,2,3}: # shmask.h d.h  shmask.h d.h
		assert(5 <= argc)
