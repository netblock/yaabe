...
"""
	...
	out_text = header_header % (name, ver_str, name, name)
...
def bitfield_to_reglist(name:str, versions:list, composite_bitfields:dict):
	# "name": [bf,bf,bf]
	header_header = """\
/*
autogenerated with asic_reg.py
...
"""
	...
	versions.sort(key=lambda v: [int(n) for n in v.split("_")])
	ver_str = ", ".join(versions)
	out_text = header_header % (name, ver_str, name, name, name)
...
def main(argc:int, argv:list):
	...
	if (mode == 0) or (mode == 1): # shmask.h d.h  shmask.h d.h
		assert(5 <= argc)
		...
		if (mode == 0):
			output_file_text = bitfield_to_header(
				ip_name, versions, composite_bitfield_sets
			)
		else:
			output_file_text = bitfield_to_reglist(
				ip_name, versions, composite_bitfield_sets
			)
